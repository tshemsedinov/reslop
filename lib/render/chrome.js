...
const headerRoleFg = (role) => THEME[HEADER_ROLE_FG[role] ?? 'headerChromeFg'];
...
const paintButtons = (layout, width, color) => {
  ...
  for (const part of layout.parts) {
    ...
    else out += paintMarkedWord(part.label, part.letter, color, buttonMark());
...
const statusDelta = (total) => {
  ...
  const plus = `+${stagedAdd}/${unstagedAdd}`;
  const minus = `-${stagedDel}/${unstagedDel}`;
...
const paintChoicePrompt = (choices, width, color, prefix = '') => {
  ...
  for (const choice of choices) {
    ...
    out += paintMarkedWord(label, choice.letter, color, quitMark());
  }
  const pad = Math.max(0, width - visibleWidth(body));
