...
const SAVE_HINT_PARTS = [
  { text: 'Press ', hot: false },
  { text: 'Ctrl-S', hot: true },
  { text: ' or ', hot: false },
  { text: 'Enter at EOF', hot: true },
  { text: ' to save', hot: false },
];

const saveHintText = () => {
  let text = '';
  for (const part of SAVE_HINT_PARTS) text += part.text;
  return text;
};
...
const paintSaveHintBar = (width, color) => {
  const bgRgb = THEME.buttonBg;
  const plain = `${BUTTON_GAP}${saveHintText()}`;
  if (!color) {
    const pad = Math.max(0, width - visibleWidth(plain));
    return plain + ' '.repeat(pad);
  }
  let out = paint(BUTTON_GAP, THEME.buttonFg, bgRgb, color);
  for (const part of SAVE_HINT_PARTS) {
    const tone = part.hot ? THEME.chromeFg : THEME.buttonFg;
