...
const statusStats = (view) => {
  ...
  const delta =
    total.added || total.removed ? `+${total.added}  -${total.removed}` : '';
...
const STATUS_MARK = /(\[[^\]]+\]|\+[0-9]+|-[0-9]+)/g;

const paintStatusMark = (token, color) => {
  const bgRgb = THEME.chromeBg;
  const muted = THEME.mutedFg;
  if (token.startsWith('[')) {
    return paintBracketName(token, THEME.buttonHotFg, muted, bgRgb, color);
  }
  if (token.startsWith('+')) {
...
const paintStatusLine = (view, status, width, color) => {
  ...
  const gap = ' '.repeat(layout.pad);
  if (!color) return layout.clipped + layout.msg + gap + layout.right;
  const bgRgb = THEME.chromeBg;
  return (
    paintStatusLeft(layout.clipped, color) +
    paint(layout.msg, layout.msgFg, bgRgb, color) +
    paint(gap, THEME.mutedFg, bgRgb, color) +
...
const statusBranchHit = (view, width, y) => {
  ...
  const open = layout.clipped.indexOf('[');
  const close = layout.clipped.indexOf(']');
