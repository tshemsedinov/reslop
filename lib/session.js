...
class Session {
  ...
  load() {
    ...
    const loaded = this.repo.load(this.cwd, this.paths, {
      commit: this.rev,
      audit: this.audit,
      outdatedMap: this.outdatedMap,
      auditMap: this.auditMap,
    });
  ...
  onFileAdd() {
    ...
    const proposed = targets.some((item) => isProposedDep(item));
  ...
  onFileRevert() {
    ...
    for (const item of items) {
      if (isProposedDep(item)) this.skipped.add(itemKey(item));
    }
    let ok = false;
    try {
