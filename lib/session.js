...
const mouseKind = (event) => {
  ...
  if (event.button === 64) return 'wheelUp';
  if (event.button === 65) return 'wheelDown';
  if (event.press && event.button & 32) return 'drag';
  if (event.press) return 'press';
  return 'release';
};
...
const itemKey = (item) => {
  ...
  if (!item.hunk) {
    ...
    return `${origin}:${path}:file:${block}`;
  }
  const oldStart = item.hunk.oldStart;
  const newStart = item.hunk.newStart;
  const block = item.blockId ?? '-';
  return `${origin}:${path}:${oldStart}:${newStart}:${block}`;
};
...
class Session {
  ...
  counts() {
    ...
    for (const item of this.items) {
      ...
      if (counts[item.origin] === undefined) continue;
  ...
  onAdd() {
    ...
    if (item.origin === 'commit') {
      this.status = 'read only';
      return;
    }
    try {
      this.repo.add(this.top, item);
      this.status = 'staged';
  ...
  onUnstage() {
    ...
    try {
      this.repo.unstage(this.top, item);
  ...
  onRevert() {
    ...
    try {
      this.repo.revert(this.top, item);
...
module.exports = { Session, HANDLERS };
