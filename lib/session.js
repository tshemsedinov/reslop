...
const { noteInnerWidth, todoInnerWidth, codeInnerWidth } = render;
...
const canEditCode = (item) => {
  if (!item || item.origin === 'todo') return false;
...
class Session {
  ...
  dispatch(action) {
    ...
    const hidden = disabledActions(this.pane, this.current());
  ...
  async startUi() {
    if (this.didLoad && !this.items.length) {
