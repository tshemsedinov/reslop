...
const { watchResize } = require('./sys.js');
const files = require('./files.js');
const { fileEntries, itemPath, TODO_FILE } = files;
const { isTodoItem, isTodosEntry, REPO_TODOS_LABEL, makeTodoItem } = files;
const { Editor } = require('./editor.js');
const { refreshIndexPatches, blockAddText } = require('./diff.js');
const update = require('./update.js');
const { checkUpdate, installUpdate, markSkipped } = update;

const { renderFrame, presentRows, presentCursor } = render;
const { noteInnerWidth, codeInnerWidth } = render;
...
const { noteCounts, hasNotes, setFeedback, setCode, addTodo } = review;
const { applyImportedNotes } = review;
const { setTodoText, removeTodo, flushReview } = review;
...
const QUIT_CONFIRM = {
  f: (session) => session.finishQuit('ready'),
  c: (session) => session.finishQuit('editing'),
  'ctrl-c': (session) => session.finishQuit('ready'),
  escape: (session) => session.cancelQuit(),
};

const COMMIT_CONFIRM = {
  c: (session) => session.startGitCommit('commit'),
  a: (session) => session.startGitCommit('amend'),
  f: (session) => session.startGitCommit('fixup'),
  escape: (session) => session.cancelCommit(),
};

const UPDATE_CONFIRM = {
  y: (session) => session.acceptUpdate(),
  n: (session) => session.declineUpdate(),
  escape: (session) => session.declineUpdate(),
};

const DROP_CONFIRM = {
  y: (session) => session.confirmDropBranch(),
  n: (session) => session.cancelDropBranch(),
  escape: (session) => session.cancelDropBranch(),
};

const PUSH_CONFIRM = {
  f: (session) => session.onForcePush(),
  escape: (session) => session.cancelPush(),
};
...
const COMPOSE_SPECIAL = {
  ...
  tab: (session) => session.composeTab(),
};
...
const HANDLERS = {
  ...
  code: (session) => session.onCode(),
  reload: (session) => session.onReload(),
  commit: (session) => session.onCommit(),
  branch: (session) => session.onBranch(),
  pull: (session) => session.onPull(),
  push: (session) => session.onPush(),
  newBranch: (session) => session.onNewBranch(),
  rebase: (session) => session.onRebaseBranch(),
  drop: (session) => session.onDropBranch(),
...
const KEY_MODE = {
  ...
  confirmCommit: (session, key) => session.onConfirmCommit(key),
  confirmUpdate: (session, key) => session.onConfirmUpdate(key),
  confirmDrop: (session, key) => session.onConfirmDrop(key),
  confirmPush: (session, key) => session.onConfirmPush(key),
...
const RELEASE_HITS = [
  {
    ...
    onHit: (session) => session.onBranch(),
  },
  {
  ...
  {
    match: (session) => session.pane === 'branches',
    hits: (frame) => frame?.fileHits,
    onHit: (session, hit) => {
      session.branchCursor = hit.cursor;
      session.onCheckoutBranch();
    },
  },
  {
...
class Session {
  assignOptions(options) {
    ...
    this.updateOpts = options.update;
  }
  ...
  shouldDeferExtras() {
    ...
    if (this.outdatedMap !== undefined && this.auditMap !== undefined) {
      return false;
    }
    return true;
  }
  ...
  applyLoaded(loaded) {
    const wasTodo = this.todoOpen === true;
    const here = itemKey(this.current());
    ...
    if (loaded.branch !== undefined) this.branch = loaded.branch || '';
    ...
    this.syncFileCursor();
    this.didLoad = true;
    this.pendingExtras = loaded.pending === true;
  }

  load() {
    const loaded = this.repo.load(this.cwd, this.paths, this.loadOptions());
  ...
  finishIfEmpty(kind) {
    ...
    if (this.notes && hasNotes(this.notes)) return;
    if (kind === 'keep') return;
    if (this.notes && hasNotes(this.notes)) this.flushReview();
    this.done = true;
    this.exitCode = 0;
    this.status = 'nothing to review';
    this.emptyReview = kind === 'open';
    if (this.finishReadLoop) this.finishReadLoop();
  }
  ...
  async followExtras(loaded, gen, kind) {
    ...
    const signal = this.loadAbort ? this.loadAbort.signal : undefined;
    ...
    let next;
    try {
      next = await this.repo.loadExtras(loaded, extra);
    } catch (error) {
      if (gen !== this.loadGen) return;
      this.failOpen(error);
      return;
    }
    if (gen !== this.loadGen || this.done) return;
    this.applyLoaded(next);
    this.clearBusy();
    this.paint();
    this.finishIfEmpty(kind);
  }
  ...
  openLoad() {
    ...
    if (this.didLoad && !this.pendingExtras) {
      this.loadPromise = Promise.resolve();
      return this.loadPromise;
    }
    this.bumpLoad();
    this.loadPromise = this.runOpenLoad();
    return this.loadPromise;
  }
  ...
  async runOpenLoad() {
    ...
    const options = this.loadOptions({
      deferExtras: this.shouldDeferExtras(),
      signal,
    });
    this.progressFrame = 0;
    ...
    let loaded;
    try {
      if (this.repo.loadAsync) {
        loaded = await this.repo.loadAsync(this.cwd, this.paths, options);
      } else {
        loaded = this.repo.load(this.cwd, this.paths, options);
      }
    } catch (error) {
      if (gen !== this.loadGen) return;
      this.failOpen(error);
    ...
    this.applyLoaded(loaded);
    if (loaded.pending) {
      this.setBusy('checking npm');
      this.paint();
      await this.followExtras(loaded, gen, 'open');
      return;
    }
    this.clearBusy();
    this.paint();
    this.finishIfEmpty('open');
  }
  ...
  updateOptions() {
    if (!this.updateOpts || this.updateOpts === true) return {};
  ...
  maybePromptUpdate() {
    ...
    this.mode = 'confirmUpdate';
  }

  openUpdate() {
    if (this.updateOpts === false) return Promise.resolve();
    if (this.updatePromise) return this.updatePromise;
    this.updatePromise = this.runOpenUpdate();
  ...
  async runOpenUpdate() {
    let plan;
    try {
      plan = await checkUpdate(this.updateOptions());
    } catch {
      return;
    }
    if (this.done) return;
    if (plan.action === 'install') {
      this.installPromise = this.runInstall(plan.latest);
      await this.installPromise;
      return;
    }
    if (plan.action !== 'confirm') return;
    this.updateFrom = plan.current;
    this.updateTo = plan.latest;
    this.updateOffer = true;
    this.paint();
  }

  async runInstall(version) {
    this.status = 'updating reslop';
    this.progressFrame = 0;
    this.startProgress();
    this.paint();
    let ok = false;
    try {
      await installUpdate(version, this.updateOptions());
      ok = true;
    } catch {
      // ignore install errors
    }
    this.stopProgress();
    if (this.done || !this.uiOpen) return;
    this.status = ok ? 'updated' : 'update failed';
    this.paint();
  }

  onConfirmUpdate(key) {
    const updateKey = key.length === 1 ? key.toLowerCase() : key;
    const handler = UPDATE_CONFIRM[updateKey];
  ...
  acceptUpdate() {
    this.mode = 'review';
    this.updateOffer = false;
    const version = this.updateTo;
    this.installPromise = this.runInstall(version);
  }
  ...
  declineUpdate() {
    ...
    this.updateOffer = false;
    this.status = '';
    markSkipped(this.updateTo, this.updateOptions());
  }
  ...
  repoNameOf() {
    return this.repoName || path.basename(this.top || this.cwd || '');
  }
  ...
  fileList() {
    ...
    const labeled = this.sourceLabel
      ? relabelStatus(entries, 'pr', this.sourceLabel)
      : relabelStatus(entries, 'commit', this.revShort);
    return [this.todosEntry(), ...labeled];
  }
  ...
  followReviewPath() {
    ...
    const item = this.current();
    const idx = isTodoItem(item)
      ? files.findIndex((entry) => isTodosEntry(entry))
      : files.findIndex(
  ...
  view() {
    ...
    const repoName = this.repoNameOf();
    return {
      item: this.current(),
      index: this.index,
      ...
      progressFrame: this.progressFrame,
      layout: this.layout,
      counts: this.counts(),
      ...
      branchCursor: this.branchCursor,
      repoName,
      ...
      codeOverlay: this.codeOverlayView(),
      templates,
  ...
  dispatch(action) {
    if (CONFIRM_MODE.includes(this.mode)) return;
    ...
    if (this.pane === 'branches') {
      if (action === 'next') return void this.onBranchMove(1);
      if (action === 'prev') return void this.onBranchMove(-1);
    }
    let target = this.current();
    if (this.pane === 'files') target = this.fileCursorEntry();
    if (this.pane === 'branches') target = this.branches[this.branchCursor];
    const hidden = disabledActions(this.pane, target);
  ...
  current() {
    if (this.todoOpen) return this.todoItem;
  ...
  refreshFromRepo(options = {}) {
    this.loadPromise = null;
    this.bumpLoad();
    if (this.uiOpen && this.repo.loadAsync) {
      return void this.refreshFromRepoAsync(options);
    }
    const loadedOpts = this.loadOptions({
      deferExtras: this.shouldDeferExtras(),
    });
    const loaded = this.repo.load(this.cwd, this.paths, loadedOpts);
    this.applyLoaded(loaded);
    this.scroll = 0;
    this.clearSelection();
    ...
    if (loaded.pending) {
      ...
      this.followExtras(loaded, this.loadGen, kind);
      return;
    }
    if (keepEmpty) return;
    this.finishIfEmpty('reload');
    if (options.doneStatus && !this.done && !this.pendingExtras) {
      this.status = options.doneStatus;
    }
  }

  async refreshFromRepoAsync(options = {}) {
    const gen = this.loadGen;
    const keepEmpty = options.keepEmpty === true;
    const loadedOpts = this.loadOptions({
      deferExtras: this.shouldDeferExtras(),
      signal: this.loadAbort ? this.loadAbort.signal : undefined,
    });
    this.progressFrame = 0;
    this.setBusy('loading');
    this.paint();
    let loaded;
    try {
      loaded = await this.repo.loadAsync(this.cwd, this.paths, loadedOpts);
    } catch (error) {
      if (gen !== this.loadGen) return;
      this.failOpen(error);
      return;
    ...
    this.clearBusy();
    if (!keepEmpty) this.finishIfEmpty('reload');
    if (options.doneStatus && !this.status && !this.busy) {
      this.status = options.doneStatus;
    }
    this.paint();
  }
  ...
  reloadAfterChange(afterLoad) {
    ...
    if (this.uiOpen && this.repo.loadAsync) {
      this.refreshFromRepo({ afterLoad: restore });
      return;
    }
  ...
  guardWritable(item) {
    ...
    if (this.readOnly || isReadOnlyOrigin(item.origin)) {
  ...
  guardFileGit(items) {
    ...
    return this.guardWritable(items[0]);
  }
  ...
  onFileAdd() {
    ...
    if (this.uiOpen && busy && this.repo.addAsync) {
      const write = async (top) => {
        for (const item of targets) {
          await this.repo.addAsync(top, item);
          this.keepOrigin(item, 'staged');
        }
      };
      const after = () => {
        if (proposed) {
          this.reloadAfterChange(() => this.selectNextFileAfter(rel));
          return;
        }
        this.selectNextFileAfter(rel);
      };
      return void this.runGitBusy(busy, 'staged', write, after);
    }
  ...
  onFileRevert() {
    ...
    this.reloadAfterChange(ok ? () => this.selectNextFileAfter(rel) : null);
  }
  ...
  onAdd() {
    ...
    if (this.uiOpen && busy && this.repo.addAsync) {
      const after = () => {
        if (needsReload(item)) this.reloadAfterChange();
        else this.keepOrigin(item, 'staged');
      };
      const write = (top) => this.repo.addAsync(top, item);
      return void this.runGitBusy(busy, 'staged', write, after);
    }
    this.applyGit(
      item,
  ...
  canCommit() {
    if (this.readOnly || this.rev || this.change) {
      this.status = 'read only';
      return false;
    }
    for (const item of this.items) {
  ...
  startGitCommit(kind) {
    if (kind !== 'amend' && !this.hasStagedChanges()) {
      this.mode = 'review';
      this.commitKind = null;
      this.status = 'nothing to commit';
      return;
    }
    this.commitKind = kind;
    let text = '';
    if (kind === 'fixup') text = 'HEAD';
    if (kind === 'amend' && this.repo.lastMessage) {
      text = this.repo.lastMessage(this.top) ?? '';
    }
    this.openCompose('commit', text);
  }

  hasStagedChanges() {
    if (this.repo.hasStaged) return this.repo.hasStaged(this.top);
    for (const item of this.items) {
  ...
  onConfirmCommit(key) {
    const commitKey = key.length === 1 ? key.toLowerCase() : key;
    const handler = COMMIT_CONFIRM[commitKey];
    if (handler) handler(this);
  }

  finishGitCommit() {
    const kind = this.commitKind;
    const message = this.editor ? this.editor.text : '';
    if (kind !== 'fixup' && !message.trim()) {
      this.status = 'empty commit message';
      return;
    }
    if (kind !== 'amend' && !this.hasStagedChanges()) {
      this.closeCompose();
    ...
    if (this.uiOpen && this.repo.commitAsync) {
      this.closeCompose();
      const write = (top) => this.repo.commitAsync(top, kind, message);
      const after = () => {
        this.dismissed.clear();
        this.refreshFromRepo({ doneStatus: done });
      };
      return void this.runGitBusy('committing', done, write, after);
    }
    try {
      this.repo.commit(this.top, kind, message);
    } catch (error) {
      this.status = error.message;
      return;
    }
    ...
    this.dismissed.clear();
    this.refreshFromRepo();
    if (!this.done && !this.pendingExtras) this.status = done;
  }

  startProgress() {
    if (this.progressTimer !== null) return;
    this.progressTimer = this.setInterval(
      () => this.tickProgress(),
      PROGRESS_MS,
    );
  }
  ...
  stopProgress() {
    if (this.progressTimer === null) return;
    this.clearInterval(this.progressTimer);
    this.progressTimer = null;
  }
  ...
  async runGitBusy(kind, done, write, after) {
    ...
    this.paint();
    try {
      await write(this.top);
    } catch (error) {
      if (error.rejected && kind !== 'force pushing') {
        this.mode = 'confirmPush';
      } else {
        this.status = error.message;
      }
    }
    ...
    if (this.mode === 'confirmPush') {
      this.paint();
      return;
    }
    if (!this.status && after) after();
    if (!this.status && !this.busy) this.status = done;
    this.paint();
  }
  ...
  onBranch() {
    ...
    if (!this.canCommit()) return;
    if (!this.repo.listBranches) {
      this.status = 'read only';
      return;
    }
    try {
      this.branches = this.repo.listBranches(this.top);
    } catch (error) {
      this.status = error.message;
      return;
    }
    const current = this.branches.findIndex((entry) => entry.current);
    this.branchCursor = current < 0 ? 0 : current;
    this.pane = 'branches';
    this.status = '';
    this.clearSelection();
  }
  ...
  onBranchMove(delta) {
    ...
    const next = Math.min(
      branches.length - 1,
      Math.max(0, this.branchCursor + delta),
    );
    if (next === this.branchCursor) return;
    this.branchCursor = next;
    this.status = '';
  }

  onCheckoutBranch() {
    if (!this.canCommit()) return;
    const entry = this.branches[this.branchCursor];
    if (!entry) return;
    if (entry.current) {
      this.showFiles();
      return;
    ...
    if (this.uiOpen && this.repo.checkoutAsync) {
      this.showFiles();
      const write = (top) => this.repo.checkoutAsync(top, name);
      const after = () =>
        this.refreshFromRepo({ keepEmpty: true, doneStatus: done });
      return void this.runGitBusy('checking out', done, write, after);
    }
    const ok = this.runRepo(done, (top) => {
      this.repo.checkout(top, name);
    });
    if (!ok) return;
    this.showFiles();
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = done;
  }

  refreshBranchList() {
    if (!this.repo.listBranches) return;
    try {
      this.branches = this.repo.listBranches(this.top);
    } catch (error) {
      this.status = error.message;
    ...
    if (this.branchCursor > last) this.branchCursor = Math.max(0, last);
  }

  onRebaseBranch() {
    if (this.pane !== 'branches') return;
    if (!this.canCommit()) return;
    const entry = this.branches[this.branchCursor];
    if (!entry || entry.current) return;
    const onto = entry.name;
    const done = `rebased onto ${onto}`;
    if (this.uiOpen && this.repo.rebaseAsync) {
      const write = (top) => this.repo.rebaseAsync(top, onto);
      const after = () => {
        this.refreshBranchList();
        this.refreshFromRepo({ keepEmpty: true, doneStatus: done });
      };
      return void this.runGitBusy('rebasing', done, write, after);
    }
    const ok = this.runRepo(done, (top) => this.repo.rebase(top, onto));
    if (!ok) return;
    this.refreshBranchList();
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = done;
  ...
  onDropBranch() {
    ...
    this.mode = 'confirmDrop';
    this.status = '';
  }

  onConfirmDrop(key) {
    const dropKey = key.length === 1 ? key.toLowerCase() : key;
    const handler = DROP_CONFIRM[dropKey];
    if (handler) handler(this);
  }
  ...
  confirmDropBranch() {
    ...
    if (this.uiOpen && this.repo.dropAsync) {
      const write = (top) => this.repo.dropAsync(top, name);
      const after = () => {
        this.refreshBranchList();
        this.refreshFromRepo({ keepEmpty: true, doneStatus: done });
      };
      return void this.runGitBusy('dropping', done, write, after);
    }
    const ok = this.runRepo(done, (top) => this.repo.drop(top, name));
    if (!ok) return;
    this.refreshBranchList();
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = done;
  ...
  onNewBranch() {
    if (this.pane !== 'branches') return;
    if (!this.canCommit()) return;
    this.openCompose('branch', '');
  }

  finishCreateBranch() {
    const name = this.editor ? this.editor.text.trim() : '';
    if (!name) {
      this.status = 'empty branch name';
      return;
    }
    const done = `created ${name}`;
    if (this.uiOpen && this.repo.createBranchAsync) {
      this.closeCompose();
      this.showFiles();
      const write = (top) => this.repo.createBranchAsync(top, name);
      const after = () =>
        this.refreshFromRepo({ keepEmpty: true, doneStatus: done });
      return void this.runGitBusy('creating branch', done, write, after);
    }
    const ok = this.runRepo(done, (top) => {
      this.repo.createBranch(top, name);
    });
    if (!ok) return;
    this.closeCompose();
    this.showFiles();
  ...
  onPull() {
    if (!this.canCommit()) return;
    if (this.repo.pullAsync) {
      const after = () => this.refreshFromRepo({ keepEmpty: true });
      const write = (top) => this.repo.pullAsync(top);
      return void this.runGitBusy('pulling', 'pulled', write, after);
    }
    const ok = this.runRepo('pulled', (top) => this.repo.pull(top));
    if (!ok) return;
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = 'pulled';
  }
  ...
  onConfirmPush(key) {
    const pushKey = key.length === 1 ? key.toLowerCase() : key;
    const handler = PUSH_CONFIRM[pushKey];
    if (handler) handler(this);
  }
  ...
  startPush(force) {
    ...
    if (this.repo.pushAsync) {
      const write = (top) => this.repo.pushAsync(top, force);
      return void this.runGitBusy(kind, done, write);
    }
    try {
      this.repo.push(this.top, force);
    } catch (error) {
  ...
  onTodo() {
    if (this.pane === 'files') {
      ...
      if (entry && !isTodosEntry(entry)) {
        this.index = entry.openIndex ?? entry.firstIndex;
      }
    }
    this.createTodo();
  }
  ...
  onCode() {
    ...
    const prev = this.notes.code.get(itemFeedKey(item));
    const original = blockAddText(item.hunk, item.blockId);
    const text = prev ? prev.text : original;
    this.openCompose('code', text);
  }

  focusTodoPage(todoId) {
    this.openTodoPage();
  ...
  editNextTodo() {
    if (!this.todoOpen) return;
    const last = this.todoRowCount() - 1;
    const next = this.clampedTodoFocus() + 1;
    if (next <= last) this.todoFocus = next;
    this.editFocusedTodo();
  }

  moveTodoCompose(delta) {
    if (!this.todoOpen) return;
    const last = this.todoRowCount() - 1;
    ...
    this.flushReview();
    this.closeCompose();
    const max = Math.max(0, this.todoRowCount() - 1);
    this.todoFocus = Math.min(next, max);
    this.editFocusedTodo();
  }

  typeIntoTodo(key) {
    if (!this.todoOpen) return false;
    if (this.pane !== 'diff') return false;
  ...
  removeFocusedTodo() {
    if (this.pane !== 'diff' || !this.todoOpen) return;
    const todos = this.listTodos();
    const focus = this.clampedTodoFocus();
    const todo = todos[focus];
    if (!todo) return;
    removeTodo(this.notes, todo.id);
    const left = this.listTodos();
    if (this.todoFocus >= left.length) {
      this.todoFocus = Math.max(0, left.length - 1);
    }
  ...
  commitCompose() {
    ...
    if (this.composeKind === 'code') this.commitCode(text);
  }
  ...
  commitCode(text) {
    ...
    if (text === original) {
      setCode(this.notes, key, { ...note, clear: true });
      return;
    }
    setCode(this.notes, key, note);
  }
  ...
  commitTodo(text) {
    ...
    if (!text.trim()) return;
    const todo = addTodo(this.notes, TODO_FILE, text);
  ...
  composeView() {
    ...
    const noteKinds = ['feedback', 'commit', 'branch'];
    if (!noteKinds.includes(this.composeKind)) return null;
  ...
  liveCodeText() {
    ...
    if (this.mode === 'compose' && this.composeKind === 'code' && this.editor) {
  ...
  codeOverlayView() {
    ...
    const editing = this.mode === 'compose' && this.composeKind === 'code';
    const cursor = editing && this.editor ? this.editor.cursor : null;
    return { text, keepEmpty: editing, cursor };
  }

  composeTab() {
    if (this.composeKind === 'code') {
      this.editor.insert('\t');
      return;
    }
    this.applyTemplate();
  ...
  todoRowCount() {
    if (!this.todoOpen) return 0;
    return this.listTodos().length + 1;
  }
  ...
  saveCompose(options) {
    const opts = options ?? {};
    if (this.composeKind === 'commit') {
      this.finishGitCommit();
      return;
    }
    if (this.composeKind === 'branch') {
      this.finishCreateBranch();
      return;
    }
    const editNext = opts.editNext === true;
    this.commitCompose();
    if (this.composeKind === 'feedback') {
    ...
    this.closeCompose();
    this.status = 'saved';
    if (editNext) this.editNextTodo();
  }

  autosave() {
    const text = this.editor ? this.editor.text : '';
    const editingCode = this.mode === 'compose' && this.composeKind === 'code';
    if (this.mode === 'compose' && (editingCode || text.trim())) {
      this.commitCompose();
    }
  ...
  moveComposeLine(delta) {
    ...
    if (this.composeKind === 'todo') {
      this.moveTodoCompose(delta);
      return;
    }
    if (this.composeKind === 'code') {
      const inner = codeInnerWidth(size.width, this.layout);
      this.editor.moveLine(delta, inner);
      return;
    }
  ...
  handleComposeKey(key) {
    ...
    if (key === 'enter') {
      ...
      if (this.composeKind === 'todo') {
        this.saveCompose({ editNext: true });
        return;
      }
      this.saveCompose();
      return;
    ...
    if (key === 'escape' || key === 'ctrl-s') {
      const cancel = ['commit', 'branch'];
      if (cancel.includes(this.composeKind) && key === 'escape') {
        this.closeCompose();
        this.status = '';
        return;
      }
  ...
  onFiles() {
    if (this.pane === 'files') {
      ...
      this.pane = 'diff';
      this.clearSelection();
      if (this.reviewPath === TODO_FILE) this.openTodoPage();
      return;
    }
    this.showFiles();
  }
  ...
  onOpenFile() {
    if (this.pane === 'branches') return void this.onCheckoutBranch();
    if (this.pane !== 'files') {
      if (this.todoOpen) this.editFocusedTodo();
      return;
    }
    const files = this.fileList();
  ...
  onFileMove(delta) {
    ...
    if (next === this.fileCursor) return;
    this.fileCursor = next;
    const entry = files[next];
    if (isTodosEntry(entry)) {
      this.reviewPath = TODO_FILE;
    } else {
      this.reviewPath = entry.path;
      this.index = entry.firstIndex;
    }
  ...
  onScroll(delta) {
    ...
    if (this.pane === 'branches') {
      this.onBranchMove(delta);
      return;
    }
    if (this.todoOpen && this.todoRowCount() > 1) {
  ...
  handleEvent(event) {
    if (event.type === 'key') {
      ...
      if (this.typeIntoTodo(event.key)) return;
      const action = actionFromKey(event.key, this.pane);
  ...
  readLoop() {
    return new Promise((resolve) => {
      ...
      watchResize(this.stdout, process, () => this.draw(), signal);
    });
  }
}
