...
class Session {
  ...
  onReload() {
    ...
    this.status = '';
    this.refreshFromRepo();
    if (!this.done && !this.pendingExtras) this.status = 'reloaded';
  }
  ...
  finishGitCommit() {
    ...
    this.dismissed.clear();
    this.refreshFromRepo();
    if (!this.done && !this.pendingExtras) {
      this.status = COMMIT_STATUS[kind] ?? 'committed';
    }
  }
  ...
  onCheckoutBranch() {
    ...
    const ok = this.runRepo(`checked out ${name}`, (top) => {
      this.repo.checkout(top, name);
    });
    if (!ok) return;
    this.showFiles();
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = `checked out ${name}`;
  }
  ...
  finishCreateBranch() {
    ...
    const ok = this.runRepo(`created ${name}`, (top) => {
    ...
    this.showFiles();
    this.refreshFromRepo({ keepEmpty: true });
    if (!this.status) this.status = `created ${name}`;
  }
