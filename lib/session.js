...
const { noteInnerWidth, QUIT_FINISH, QUIT_CONTINUE } = render;
...
const QUIT_CONFIRM = Object.assign(Object.create(null), {
  [QUIT_FINISH]: (session) => session.finishQuit('pending'),
  [QUIT_CONTINUE]: (session) => session.finishQuit('editing'),
  'ctrl-c': (session) => session.finishQuit('pending'),
  escape: (session) => session.cancelQuit(),
});

const COMPOSE_EDITS = ['backspace', 'delete', 'ctrl-z', 'ctrl-y'];

const COMPOSE_SPECIAL = Object.assign(Object.create(null), {
...
class Session {
  ...
  injectTodos() {
    ...
    this.items = mergeTodos(this.items, todos);
  }
  ...
  onTodoHit(cursor) {
    ...
    if (this.mode === 'compose' && this.composeKind === 'todo') {
      if (target.id === this.composeTodoId) return;
      this.commitCompose();
      this.closeCompose();
    }
  ...
  createTodo(file) {
    if (!file) return;
    const todo = addTodo(this.notes, file, '');
    this.injectTodos();
    this.pane = 'diff';
    this.help = false;
    this.scroll = 0;
    this.clearSelection();
    this.focusTodoPage(file, todo.id);
    this.syncReviewPath();
    this.openCompose('todo', '', todo.id);
  }
  ...
  commitCompose() {
    ...
    if (this.composeKind === 'todo' && this.composeTodoId !== null) {
      this.commitTodo(text);
    }
  }

  commitTodo(text) {
    const id = this.composeTodoId;
    const prev = this.notes.todos.find((entry) => entry.id === id);
    const file = prev ? prev.file : itemPath(this.current());
    if (prev) {
  ...
  todoTexts() {
    ...
    for (const todo of this.fileTodos(itemPath(item))) {
      texts.push(checkLabel(todo.text, todo.done));
    }
    return texts;
  }
  ...
  moveTodoFocus(delta) {
    const item = this.current();
    if (!item || item.origin !== 'todo') return;
    const n = this.fileTodos(itemPath(item)).length;
    if (!n) return;
