...
class Session {
  ...
  dispatch(action) {
    ...
    const target =
      this.pane === 'files' ? this.fileCursorEntry() : this.current();
  ...
  removeReviewFile() {
    const file = this.notes && this.notes.reviewPath;
    if (!file) return;
    try {
      this.unlinkSync(file);
    } catch {
      // ignore missing review file
    }
  }

  discardQuit() {
    if (this.notes) this.notes.dirty = false;
    if (!this.didResumeReview) this.removeReviewFile();
    this.done = true;
    this.exitCode = 0;
  }
