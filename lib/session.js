...
const assignOptions = (session, options) => {
  ...
  session.watchOpts = {
    watch: options.watch,
    readdirSync: options.watchReaddir,
    statSync: options.watchStat,
    setTimeout: options.setTimeout ?? setTimeout,
    clearTimeout: options.clearTimeout ?? clearTimeout,
    debounceMs: options.watchDebounceMs,
    recursive: options.watchRecursive,
  };
...
const assignControllers = (session) => {
  ...
  session.changeActions = createChangeActions({
    ...
    ignoreWatch: () => session.lifecycle.ignoreWatch(),
  });
...
const bindFilesPane = (session) => {
  session.filesPane = createFilesPane({
    ...
    onBranchMove: session.gitBranches.onBranchMove,
    onCommitMove: session.commits.onCommitMove,
    ...
    top: () => session.top,
    rev: () => session.rev,
    liveFileText: () => session.composer.liveFileText(),
  });
};
...
const bindDispatch = (session) => {
  ...
  session.dispatcher = createDispatcher({
    ...
    currentTarget: () => {
      ...
      if (session.pane === 'commits') {
        const entry = session.commits.commits[session.commitCursor];
        const staged = targetHasStaged(session);
    ...
    commits: session.commits,
    gitBranches: session.gitBranches,
    onFileMove: filesPane.onFileMove,
    onBranchMove: session.gitBranches.onBranchMove,
    onCommitMove: session.commits.onCommitMove,
    ...
    onFileScope: filesPane.onFileScope,
    onDiffScope: filesPane.onDiffScope,
    onReload: session.lifecycle.onReload,
    onScroll: filesPane.onScroll,
    onHome: filesPane.onHome,
    onEnd: filesPane.onEnd,
    scrollStep: filesPane.scrollStep,
  });
  session.pointer = createPointer({
    ...
    getPane: () => session.pane,
    getMode: () => session.mode,
    confirmChoice: (key) => session.input.confirmChoice(key),
  ...
  session.input = createInput({
    getMode: () => session.mode,
    getPane: () => session.pane,
    getFileScope: () => session.fileScope,
    composer: session.composer,
    ...
    drop: {
      confirm: () => {
        if (session.commits.dropName) {
          return void session.commits.confirmDropCommit();
        }
        session.gitBranches.confirmDropBranch();
      },
      cancel: () => {
        if (session.commits.dropName) {
          return void session.commits.cancelDropCommit();
        }
        session.gitBranches.cancelDropBranch();
      },
    },
    push: {
