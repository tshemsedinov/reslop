...
class Session {
  ...
  dispatch(action) {
    ...
    if (this.mode === 'confirmCommit') return;
    if (this.mode === 'confirmUpdate') return;
  ...
  onPush() {
    ...
    if (this.repo.pushAsync) {
      const write = (top) => this.repo.pushAsync(top);
      return void this.runGitBusy('pushing', 'pushed', write);
    }
    this.runRepo('pushed', (top) => this.repo.push(top));
  }
