...
const pushAuditNotes = (rows, audit) => {
  ...
  rows.push(ctxLine(''));
  for (const text of texts) rows.push(warnLine(text));
};
...
const toHunkLine = (row, blockId) => {
  ...
  const text = row.text;
  const noNl = false;
  const id = type === 'ctx' || type === 'warn' ? null : blockId;
...
const viaNotes = (via, pkgName) => {
  ...
  for (const item of via) {
    ...
    const note = named ? `${named}: ${title}` : title;
...
const parseAuditReport = (text) => {
  ...
  if (advisories && typeof advisories === 'object') {
    for (const entry of Object.values(advisories)) {
      ...
      const title = name && raw ? `${name}: ${raw}` : raw;
...
const buildProposedChanges = (pkg, outdated, audit, lock) => {
  const names = new Set();
  if (outdated) {
...
const proposeDepItems = (pkgText, outdated, audit, extra = {}) => {
  ...
  const changes = buildProposedChanges(pkg, outdated, audit, lock);
