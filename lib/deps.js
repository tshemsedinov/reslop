...
const { relOf, listFilePath } = manifest;
...
const { foldedDepNames, stripFoldedDepItem } = changes;
...
const groupDir = (key) => key.slice(key.indexOf('\0') + 1);

const groupedSynthetics = (synthetic) => {
  const byPath = new Map();
  for (const key of synthetic.keys()) {
    const rel = relOf(groupDir(key), MANIFEST);
...
const spliceAtSlots = (out, synthetic, slot) => {
  ...
  for (const rel of byPath.keys()) {
    ...
    inserts.push({ at, items: byPath.get(rel) });
  }
