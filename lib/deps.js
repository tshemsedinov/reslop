...
const sectionTitle = (section, action, marks) => {
...
const buildRows = (changes, oldCount, newCount) => {
  ...
  for (const bucket of buckets) {
    ...
    const title = sectionTitle(bucket.section, bucket.action, marks);
...
const keepHighestAudit = (audit, name, severity, title) => {
  if (!name || !severity || severity === 'info') return;
  const prev = audit.get(name);
  if (prev && severityRank(prev.severity) >= severityRank(severity)) return;
  audit.set(name, { severity, title });
};
...
const parseAuditReport = (text) => {
  ...
  if (vulns && typeof vulns === 'object') {
    for (const [key, entry] of Object.entries(vulns)) {
      ...
      keepHighestAudit(audit, name, severity, title);
    }
  }
