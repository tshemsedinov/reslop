...
const SKIP_FIELDS = new Set(SECTION_KEYS);
const ACTION_TITLE = {
  added: 'added',
  removed: 'removed',
...
const ACTION_ORDER = ['added', 'removed', 'changed'];
...
const sectionTitle = (section, action) => {
  const title = ACTION_TITLE[action] ?? action;
  if (section === 'field') return `package.json ${title}`;
  if (section === 'resolved') return `lockfile resolved ${title}`;
...
const buildRows = (changes, oldCount, newCount) => {
  ...
  for (const bucket of buckets) {
    ...
    const title = sectionTitle(bucket.section, bucket.action);
    rows.push(ctxLine(title));
    for (const change of bucket.items) pushChangeLines(rows, change);
  }
...
const foldDepItems = (items, readSides = noSides) => {
