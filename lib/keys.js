...
const ACTIONS = {
  add: { keys: ['a'], label: 'Add' },
  unstage: { keys: ['u'], label: 'Unstage' },
  revert: { keys: ['d'], label: 'Drop' },
  commit: { keys: ['c'], label: 'Commit' },
  prev: { keys: ['left', 'p', 'k'], hot: '←', label: '←' },
  next: { keys: ['right', 'n', 'j'], hot: '→', label: '→' },
  layout: { keys: ['m'], label: 'Mode' },
  feedback: { keys: ['f'], label: 'Feedback' },
  todo: { keys: ['t'], label: 'Todo' },
  code: { keys: ['e'], label: 'Edit' },
  reload: { keys: ['r'], label: 'Reload' },
  quit: { keys: ['q', 'ctrl-c'], label: 'Q' },
};
const ACTION_IDS = Object.keys(ACTIONS);

const DIFF_DISABLED = ['reload'];
...
for (const id of ACTION_IDS) {
  for (const key of ACTIONS[id].keys) KEY_TO_ACTION[key] = id;
}
