...
const keepOrigin = (options, item, origin) => {
  ...
  const idx = options.collection.findRestoredIndex({ ...item, origin });
  if (idx >= 0) options.nav.index = idx;
  if (options.ignoreWatch) options.ignoreWatch();
...
const hunkActionItems = (options, item) => {
...
const onUnitAdd = (options, targets) => {
  const proposed = targets.some(needsReload);
  const busy = targets.map(npmBusyKind).find((kind) => kind);
  const repo = options.repo();
  if (options.uiOpen() && busy && repo.addAsync) {
  ...
  const result = applyFileGit(
    ...
    'staged',
    (top, current) => repo.add(top, current),
    (entry) => keepOrigin(options, entry, 'staged'),
  );
  if (reconcilePartial(options, result)) return;
  if (proposed) options.reloadAfterChange();
};

const onUnitUnstage = (options, targets) => {
  const repo = options.repo();
  const result = applyFileGit(
    options,
    targets,
    'unstaged',
    (top, current) => repo.unstage(top, current),
    (entry) => keepOrigin(options, entry, nextUnstageOrigin(entry)),
  );
  reconcilePartial(options, result);
};

const onUnitRevert = (options, targets) => {
  for (const item of targets) {
    if (needsReload(item)) options.collection.dismiss(item);
  }
  const repo = options.repo();
...
const onUnitCommand = (options, name) => {
  const command = COMMANDS[name];
  const item = activeDiffItem(options);
  if (!guardWritable(options, item)) return;
  const selected = hunkActionItems(options, item);
  const targets = name === 'revert' ? selected : selected.filter(command.match);
  if (!targets.length) {
    options.setStatus(command.skipStatus ?? 'not a diff block');
    return;
  }
  UNIT[name](options, targets);
};

const onCommand = (options, name) => {
  if (options.nav.pane === 'files') return void FILE[name](options);
  if (options.nav.pane === 'unit') return void onUnitCommand(options, name);
