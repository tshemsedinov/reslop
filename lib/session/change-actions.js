...
const strategyOf = (item) => {
  if (needsReload(item)) return 'reload';
  if (item.file && item.file.isBinary) return 'binary';
  return 'patch';
};

const usesReload = (item) => strategyOf(item) === 'reload';
...
const onFileAdd = (api) => {
  ...
  const proposed = targets.some((item) => usesReload(item));
...
const createChangeActions = (options) => {
  ...
  return {
    ...
    fileActionItems: () => fileActionItems(api),
    onAdd: () => onCommand(api, 'add'),
    onUnstage: () => onCommand(api, 'unstage'),
    onRevert: () => onCommand(api, 'revert'),
    onFileAdd: () => onFileAdd(api),
    onFileUnstage: () => onFileUnstage(api),
    onFileRevert: () => onFileRevert(api),
  };
};

module.exports = { COMMANDS, strategyOf, createChangeActions };
