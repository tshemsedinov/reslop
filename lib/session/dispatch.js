...
const createDispatcher = (options) => {
  const handlers = {
    ...
    amend: options.commits.onAmend,
    fixup: options.commits.onFixup,
    ...
    drop: () => {
      if (options.getPane() === 'commits') {
        return void options.commits.onDropCommit();
      }
      options.gitBranches.onDropBranch();
    },
    open: options.onOpenFile,
    file: options.onFileScope,
    diff: options.onDiffScope,
  ...
  const dispatch = (action) => {
    ...
    if (pane === 'commits') {
      if (action === 'next') return void options.onCommitMove(1);
      if (action === 'prev') return void options.onCommitMove(-1);
    }
...
const createInput = (options) => {
  ...
  const handleKey = (key) => {
    ...
    const action = actionFromKey(
      key,
      options.getPane(),
      options.getFileScope && options.getFileScope(),
    );
