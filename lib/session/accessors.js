...
const stateView = (state, methods, writable = [], readonly = []) => {
  const descriptors = Object.getOwnPropertyDescriptors(methods);
  const view = Object.defineProperties({}, descriptors);
  bindAccessors(view, state, writable);
  bindGetters(view, state, readonly);
  return view;
};

module.exports = { bindAccessors, bindGetters, bindAlias, stateView };
