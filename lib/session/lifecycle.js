...
const { itemPath, relativeAge } = files;
const { MANIFEST, LOCKFILE } = require('../deps.js');
...
const npmStamp = (api) => {
  ...
  const pkg = fileStamp(top, MANIFEST);
  const lock = fileStamp(top, LOCKFILE);
  return `${pkg}|${lock}`;
};
...
const currentFileStamp = (api) => {
  const pane = api.nav.pane;
  if (pane !== 'diff' && pane !== 'unit') return '';
  const rel = pane === 'unit' ? api.nav.reviewPath : itemPath(api.ui.current());
  if (!rel) return '';
