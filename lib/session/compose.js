...
const { itemPath, isTodoItem, listPath } = files;
...
const unitOpts = (api) => ({
  ...
  top: () => api.ui.top,
  rev: () => api.ui.rev,
  liveFileText: () => (api.state.editor ? api.state.editor.text : null),
  editRows: () => api.state.fileEditRows,
});

const followFileEdit = (api) => {
  if (api.state.composeKind !== 'file') return;
  const rows = unit.linesFor(unitOpts(api), api.nav.reviewPath);
...
const applyDiskText = (api, text) => {
  ...
  api.state.composeBaseline = text;
  const opts = unitOpts(api);
  opts.liveFileText = () => null;
  api.state.fileEditRows = unit.linesFor(opts, api.nav.reviewPath);
  followFileEdit(api);
};
