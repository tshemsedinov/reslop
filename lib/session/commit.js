...
const cancelCommit = (api) => {
  ...
  api.options.composer.commitKind = null;
  api.options.setStatus('');
};
...
const startGitCommit = (api, kind) => {
  ...
  api.options.composer.commitKind = kind;
  let text = '';
...
const onConfirmCommit = (api, key) => {
  const action = confirmKey('confirmCommit', key);
  if (action === 'cancel') {
    cancelCommit(api);
    return;
  }
  if (action) startGitCommit(api, action);
};

const finishAsyncCommit = (api, kind, message, done) => {
  const composer = api.options.composer;
  composer.closeCompose();
  const picked = pickRepoWrite(api.options.repo(), 'commit', true);
  const write = (top) => picked.write(top, kind, message);
  const after = () => {
    api.options.collection.clearDismissed();
    api.options.refreshFromRepo({ doneStatus: done });
  };
  return void api.options.runner.runBusy('committing', done, write, after);
};
...
const finishGitCommit = (api) => {
  const kind = api.options.composer.commitKind;
  ...
  if (picked.async) {
    finishAsyncCommit(api, kind, message, done);
    return;
  }
