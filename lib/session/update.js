...
const createUpdateCoordinator = (options) => {
  ...
  return {
    ...
    reset: () => reset(state),
    enabled: () => options.updateOpts() !== false,
    checkUpdate: () => checkUpdate(optsOf(api)),
    install: (version) => installUpdate(version, optsOf(api)),
    skip: (version) => markSkipped(version, optsOf(api)),
    setCheckPromise(promise) {
      state.checkPromise = promise;
      return promise;
    },
    offerConfirm(from, to) {
      state.from = from;
      state.to = to;
      state.offer = true;
    },
    ...
    open: () => openUpdate(api),
    runInstall: (version) => runInstall(api, version),
  };
};
