...
const onEscape = (options) => {
  if (options.nav.pane === 'branches' || options.nav.pane === 'commits') {
    showFiles(options);
    return;
  }
  if (options.nav.pane === 'files') {
...
const onFiles = (options) => {
  if (options.nav.pane === 'files') {
    ...
    if (options.nav.fileScope === 'file') {
      const list = options.fileList();
      const entry = list[options.nav.fileCursor];
      if (entry && !isTodosEntry(entry)) openUnitFile(options, entry);
      return;
    }
...
const onOpenFile = (options) => {
  ...
  if (options.nav.pane === 'commits') return;
  if (options.nav.pane !== 'files') {
  ...
  if (options.nav.fileScope === 'file') {
    return void openUnitFile(options, entry);
  }
  options.nav.pane = 'diff';
...
const onFileMove = (options, delta) => {
  ...
  const moved = options.nav.fileCursor + delta;
...
const onNext = (options) => {
  if (options.nav.pane === 'unit') return void moveUnitBlock(options, 1);
...
const onScroll = (options, delta) => {
  if (options.nav.pane === 'files') return void onFileMove(options, delta);
  if (options.nav.pane === 'branches') {
    return void options.onBranchMove(delta);
  }
  if (options.nav.pane === 'commits') {
    return void options.onCommitMove(delta);
  }
  if (options.nav.todoOpen && options.composer.todoRowCount() > 1) {
    return void options.composer.moveTodoFocus(delta);
  }
  const frame = options.lastFrame();
  const max = frame ? frame.scrollMax : null;
  const scroll = options.nav.scroll;
...
const createFilesPane = (options) => ({
  ...
  onFileScope: () => setFileScope(options, 'file'),
  onDiffScope: () => setFileScope(options, 'diff'),
});
