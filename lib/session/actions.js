...
const DIFF_DISABLED = ['commit', 'branch', 'pull', 'push'];
const FILES_DISABLED = ['layout', 'feedback', 'code', 'pull', 'push'];
...
const commitWriteDisabled = (item) => {
  if (!item || !item.sha) return ['amend', 'fixup', 'drop'];
  return [];
};
...
const actionFromKey = (key, pane) => {
  const listPane = pane === 'branches' || pane === 'commits';
