...
const highlightJson = (source) => {
  ...
  while (i < n) {
    ...
    if (/[a-z]/.test(ch)) {
      ...
      const isLit = word === 'true' || word === 'false' || word === 'null';
      emit(out, isLit ? 'literal' : 'plain', word);
      i = j;
      continue;
    }
    if (/[{}[\],:]/.test(ch)) {
...
const overlayTokens = (tokens, spans) => {
  ...
  while (si < syn.length && di < diff.length) {
    ...
    const sText = s.text ?? '';
    const dText = d.text ?? '';
    const sLeft = sText.length - sOff;
    const dLeft = dText.length - dOff;
    const n = Math.min(sLeft, dLeft);
    if (n > 0) {
      const name = typeof s.style === 'string' ? s.style : 'plain';
      pieces.push({
        text: sText.slice(sOff, sOff + n),
        style: name || 'plain',
    ...
    sOff += n;
    dOff += n;
    if (sOff >= sText.length) {
      si += 1;
      sOff = 0;
    }
    if (dOff >= dText.length) {
      di += 1;
      dOff = 0;
    }
  }
  while (di < diff.length) {
    const d = diff[di];
    const text = (d.text ?? '').slice(dOff);
    if (text) {
      pieces.push({ text, style: 'plain', changed: Boolean(d.changed) });
    }
    di += 1;
    dOff = 0;
  }
