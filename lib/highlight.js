...
const DART_LITERALS = ['true', 'false', 'null', 'this', 'super'];

const DART_TYPES = ['void', 'dynamic', 'Never', 'int', 'double', 'num', 'bool'];
...
const BASH_WORD_STYLE = {
  ...wordStyle(BASH_KEYWORDS, 'control'),
  ...wordStyle(BASH_BUILTINS, 'function'),
};

const DART_WORD_STYLE = {
  ...wordStyle(DART_CONTROL, 'control'),
  ...wordStyle(DART_LITERALS, 'literal'),
  ...wordStyle(DART_STORAGE, 'storage'),
  ...wordStyle(DART_TYPES, 'type'),
};

const DART_NEXT_EXPECT = {
  class: 'class',
  mixin: 'class',
  enum: 'class',
  extension: 'class',
...
const highlightJsFamily = (source, options = {}) => {
  ...
  while (i < n) {
    ...
    if (ch === '`') {
      const r = highlightTemplate(source, i, inner);
    ...
    if (jsx && ch === '<' && isJsxTagStart(source, i)) {
      const node = highlightJsxElement(source, i, inner);
...
const highlightJs = (source) => highlightJsFamily(source, {});
const highlightTs = (source) => highlightJsFamily(source, { ts: true });
const highlightJsx = (source) => highlightJsFamily(source, { jsx: true });
const highlightTsx = (source) =>
  highlightJsFamily(source, { ts: true, jsx: true });
...
const dartIdentStyle = (ctx) => {
  const known = DART_WORD_STYLE[ctx.word];
  if (known) return known;
...
const highlightDart = (source) => {
  const inner = (expr) => highlightDart(expr);
  const out = [];
  let i = 0;
  const n = source.length;
  let expectName = null;
  while (i < n) {
    ...
    if (/[A-Za-z_$]/.test(ch)) {
      ...
      if (EXPECT_STYLE[expectName]) expectName = null;
      else expectName = DART_NEXT_EXPECT[word] ?? null;
      i = end;
      continue;
    }
    if (/[=<>!+\-*/%&|^~?:]/.test(ch)) {
