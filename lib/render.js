...
const { ACTIONS, actionHot, actionLetter, buttonWord } = keys;
...
const originLabel = (origin, revShort, sourceLabel) => {
  if (origin === 'commit') return revShort || 'commit';
  if (origin === 'pr') return sourceLabel || 'pr';
...
const headerTarget = (view) => {
  ...
  return {
    ...
    origin: originLabel(view.item.origin, view.revShort, view.sourceLabel),
    n: view.index + 1,
    total: view.total,
  };
};
...
const helpLines = () => {
  ...
  lines.push(' enter            open file / save note');
  lines.push(' esc              files, then quit');
...
const layoutButtons = (width, compact = false, disabled = []) => {
  const build = (short) => {
    ...
    for (const action of ACTIONS) {
      ...
      const isDisabled = disabled.includes(action.id);
      if (!isDisabled) hits.push({ id: action.id, x0: x, x1: x + w });
...
const expandWarnRows = (lines, width, split) => {
  const wrapWidth = split ? splitWidths(width).leftW : width;
  const out = [];
  for (const row of lines) {
    const line = split ? row.left || row.right : row;
    if (!line || line.type !== 'warn') {
      out.push(row);
      continue;
    }
    const parts = wrapWarnLine(line, wrapWidth);
...
const paintBodyDiff = (item, layout, width, color) => {
  ...
  const lines = expandWarnRows(raw, width, splitBody);
  const body = [];
  if (splitBody) {
    for (const row of lines) {
