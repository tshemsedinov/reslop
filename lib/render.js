...
const QUIT_FINISH_LABEL = 'finish review';
...
const paintTodoList = (texts, focus, width, color) => {
  ...
  for (let i = 0; i < texts.length; i++) {
    ...
    const fgRgb = selected ? THEME.chromeFg : THEME.mutedFg;
    const bgRgb = selected ? THEME.buttonBg : THEME.ctxBg;
    const wrapped = wrapMultiline(texts[i], inner);
...
const paintNotePanel = (view, width, color, cap) => {
  ...
  if (view.compose) return paintComposeNote(view, width, color, cap);
...
const paintBodyTodo = (view, width, color) => {
  ...
  const painted = paintTodoList(lines, view.todoFocus ?? 0, width, color);
  return { body: painted.rows, todoOwners: painted.owners };
};
...
const windowBody = (view, width, color, bodyH, headerLines, paintedBody) => {
  ...
  return { slice, scroll, todoHits, fileHits: paintedBody.fileHits };
};
