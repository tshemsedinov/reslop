...
const BUSY_BAR = ['▰▰▱▱▱▱', '▱▰▰▱▱▱', '▱▱▰▰▱▱', '▱▱▱▰▰▱', '▱▱▱▱▰▰', '▰▱▱▱▱▰'];
const BUSY_PROGRESS = ['pulling', 'pushing'];
...
const INFO_PREFIX = ['checked out ', 'created '];
...
const layoutButtons = (width, compact = false, hidden = [], extra = []) => {
  const ids = [];
  ...
  const build = (short) => {
    ...
    for (const id of ids) {
      ...
      parts.push({ action: { id }, piece, letter, label });
      x += w;
    }
...
const paintBranchRow = (entry, width, color, selected, cols) => {
  ...
  const bgRgb = selected ? THEME.buttonBg : THEME.ctxBg;
  const nameFg = entry.isDefault ? THEME.headerFg : THEME.buttonHotFg;
  const nameBg = entry.isDefault ? THEME.buttonHotFg : bgRgb;
  const edge = fill(1, fgRgb, bgRgb, color);
  if (!color) return `${left}${suffix}${edge}`;
  let out = `${seq(fgRgb, bgRgb)}${EL}`;
  out += paint(prefix, fgRgb, bgRgb, color);
  out += paintBracketName(label, nameFg, THEME.mutedFg, nameBg, color);
...
const renderFrame = (view, options = {}) => {
  ...
  const extra = view.pane === 'branches' ? Object.keys(BRANCH_ACTIONS) : [];
  const layout = layoutButtons(width, false, hiddenActions(view), extra);
