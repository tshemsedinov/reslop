...
const layoutButtons = (width, compact = false, hidden = []) => {
  const ids = ACTION_IDS.filter((id) => !hidden.includes(id));
  const build = (short) => {
    const hits = [];
    const parts = [];
    let x = 0;
...
const paintChoicePrompt = (choices, width, color) => {
  const bgRgb = THEME.chromeBg;
  let body = choices.map((choice) => `${choice.lead}${choice.label}`).join('');
  if (color) {
    body = choices
      .map((choice) => {
        const label = `${choice.lead}${choice.label}`;
        return paintMarkedWord(label, choice.letter, color, QUIT_MARK);
      })
      .join('');
  }
...
const paintNotePanel = (view, width, color, cap) => {
  const empty = { rows: [], cursor: null, templateHits: [] };
  const compose = view.compose;
  if (compose && (compose.kind === 'feedback' || compose.kind === 'commit')) {
...
const frameResult = (text, painted, layout, windowed, hits, bodyH) => {
...
const hiddenActions = (view) => {
  const pane = view.pane ?? 'diff';
  return disabledActions(pane, view.item);
};
...
const renderFrame = (view, options = {}) => {
  ...
  const layout = layoutButtons(width, false, hiddenActions(view));
  ...
  return frameResult(text, painted, layout, windowed, hits, bodyH);
};
