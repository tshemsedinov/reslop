...
const { foldDepItems, mergeDepFile, depFileMeta } = require('./deps.js');
...
const foldLoaded = (items, top, rev) => {
  const readSides = readDepSides(top, rev);
  return foldDepItems(items, readSides);
};
...
const load = (cwd, paths = [], options = {}) => {
  ...
  const items = foldLoaded(merged, top);
  return { top, items };
};
