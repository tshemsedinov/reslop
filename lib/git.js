...
const pushChanges = (top) => {
  const result = runGit(['push'], top);
  if (result.status === 0) return;
  ...
  requireOk(runGit(args, top), 'push failed');
};

const pushChangesAsync = async (top) => {
  const result = await runGitAsync(['push'], top);
  if (result.status === 0) return;
  const msg = result.stderr || result.stdout || 'push failed';
  if (!msg.toLowerCase().includes('no upstream')) {
    throw new Error(oneLine(msg));
  ...
  const args = ['push', '-u', 'origin', 'HEAD'];
  requireOk(await runGitAsync(args, top), 'push failed');
};

const commitChanges = (top, kind, message) => {
  if (kind === 'fixup') {
    const target = (message ?? '').trim() || 'HEAD';
    const result = runGit(['commit', `--fixup=${target}`], top);
    requireOk(result);
    return;
  }
  const args = ['commit'];
  ...
  const result = runGit(args, top, body);
  requireOk(result);
};
