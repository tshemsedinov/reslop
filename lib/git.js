...
const foldLoaded = (items, top, rev, extra = {}) => {
  ...
  if (hasDepDiff || live) usedNames = collectUsedNames(top);
  if (live) {
...
const runNpmUninstall = (cwd, name) =>
  spawnSync('npm', ['uninstall', name], {
    cwd,
    encoding: GIT_ENCODING,
    maxBuffer: GIT_MAX_BUFFER,
    timeout: INSTALL_MS,
...
const applyProposedUpdate = (top, item) => {
  ...
  let fallback = runNpmInstall;
  if (removing) fallback = (dir) => runNpmUninstall(dir, change.name);
  else if (lockOnly) fallback = runNpmAuditFix;
