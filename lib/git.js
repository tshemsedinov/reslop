...
const applyProposedUpdate = (top, item) => {
  ...
  if (!manifest) manifest = files[0] || '';
  const meta = depFileMeta(manifest);
  if (!meta) return;
  ...
  const install = item.dep.install ?? runNpmInstall;
  const result = install(cwd);
  if (result && (result.error || result.status !== 0)) {
    writeWorktreeFile(top, manifest, worktree);
    if (result.error) {
  ...
  const toAdd = [manifest];
  const lockRel = depRel(meta.dir, LOCKFILE);
  if (fs.existsSync(path.join(top, lockRel))) toAdd.push(lockRel);
