...
const USAGE = 'Usage: reslop [-n] [-r] [-v] [path | commit | pr-url | mr-url]';
...
/**
 * Parse command line arguments after the executable and script names.
 *
 * Flags: `-n` starts a new review, `-r` opens read-only, `-v` or `--version`
 * prints the version. Any other argument starting with `-` throws
 * `unknown option`. The version flag returns as soon as it is seen, so
 * arguments after it are never validated. Everything else is a path or
 * revision and is collected in order.
 *
...
const parseArgv = (argv) => {
  const args = { paths: [], newReview: false, readOnly: false, version: false };
...
const openRemoteSession = async (proc, deps, args, selected, env) => {
