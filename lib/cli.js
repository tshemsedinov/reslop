...
const { THEME_NAMES, setTheme } = require('./ansi.js');
...
const themeArg = (value) => {
  if (THEME_NAMES.includes(value)) return value;
  if (value === undefined || value === '') throw new Error('missing theme');
...
const parseArgv = (argv) => {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--theme') {
      i += 1;
      theme = themeArg(argv[i]);
      continue;
    }
    if (arg.startsWith('--theme=')) {
      theme = themeArg(arg.slice('--theme='.length));
      continue;
    }
    if (arg === '-n') {
      newReview = true;
  ...
  return { paths, newReview, readOnly, theme };
};
...
const openLocalSession = (proc, deps, args, selected, env) => {
  ...
  const scope = resolveScope(cwd, selected.paths, rawRepo, statSync);
