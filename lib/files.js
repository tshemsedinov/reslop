...
const { listFilePath } = require('./deps/manifest.js');
...
const relativeAge = (thenMs, nowMs = Date.now()) => {
  ...
  if (diff < 90) return countAgo(diff, 'second');
  const minutes = Math.round(diff / 60);
  if (minutes < 90) return countAgo(minutes, 'minute');
  const hours = Math.round(minutes / 60);
  if (hours < 36) return countAgo(hours, 'hour');
  const days = Math.round(hours / 24);
  if (days < 14) return countAgo(days, 'day');
  if (days < 70) return countAgo(Math.round(days / 7), 'week');
  if (days < 365) return countAgo(Math.round(days / 30), 'month');
  return countAgo(Math.round(days / 365), 'year');
};
