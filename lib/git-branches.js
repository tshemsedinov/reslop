...
const COMMIT_FORMAT = ['%H', '%h', '%an', '%ar', '%s'].join('%x1f');
const COMMIT_LIMIT = 256;
const UNIT = '\x1f';

const parseCommitLine = (line, index) => {
  ...
  return {
    ...
    author: (parts[2] ?? '').trim(),
    date: (parts[3] ?? '').trim(),
    subject: parts.slice(4).join(UNIT).trim(),
    head: index === 0,
  };
};
...
const listCommits = (top) => {
  ...
  for (const line of result.stdout.split('\n')) {
    if (!line) continue;
    const entry = parseCommitLine(line, commits.length);
...
const ancestorWithSubject = (top, sha, want) => {
  const args = ['log', `--format=${COMMIT_FORMAT}`, `${sha}^`];
  const result = runGit(args, top);
  if (result.status !== 0) return '';
  for (const line of result.stdout.split('\n')) {
    if (!line) continue;
    const entry = parseCommitLine(line, 1);
    if (entry && entry.subject === want) return entry.sha;
  }
  return '';
};
