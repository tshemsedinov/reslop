...
const { REVIEW_DIR, TODO_FILE } = require('./files.js');
...
const storeCode = (store, note, key) => {
  const next = {
    file: note.file,
    oldStart: note.oldStart ?? 0,
    newStart: note.newStart ?? 0,
  ...
  store.code.set(key || feedbackKey(next), next);
};
...
const groupReviewNotes = (store) => {
  ...
  for (const key of store.feedback.keys()) {
    const note = store.feedback.get(key);
    if (!note.text.trim()) continue;
    fileGroup(groups, note.file).feedback.push({ key, note });
  }
  for (const key of store.code.keys()) {
    const note = store.code.get(key);
...
const storeCodeNote = (store, pending, text) => {
  storeCode(store, {
    file: pending.file,
...
const flushPendingCode = (store, pending) => {
  if (!pending) return;
  const text = pending.open ? pending.lines.join('\n') : '';
  storeCodeNote(store, pending, text);
};

const takeCodeLine = (store, pending, line) => {
  ...
  if (!pending.open) {
    ...
    storeCodeNote(store, pending, '');
    return 'fallthrough';
  }
  if (fence && fence[1].length >= pending.open.length) {
    storeCodeNote(store, pending, pending.lines.join('\n'));
    return null;
  }
...
const parseReview = (text, reviewPath, templates = []) => {
  ...
  for (const line of text.split('\n')) {
    if (pendingCode) {
      const nextCode = takeCodeLine(store, pendingCode, line);
  ...
  flushPendingCode(store, pendingCode);
  flushPendingFeedback(store, pending);
  store.dirty = false;
  return store;
};
