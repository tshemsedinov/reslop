...
const AGENT_INSTRUCTIONS = [
  ...
  'Set frontmatter `status` to `partial` if some remain, or `done` if all',
  'are `[x]`. Do not redo items already marked `[x]`.',
].join('\n');
...
const reviewHeader = (store) => {
  ...
  const rawStatus = store.status || 'editing';
  const status = REVIEW_STATUSES.includes(rawStatus) ? rawStatus : 'editing';
  return [
    '---',
    `status: ${status}`,
    '---',
    '',
    `# metadiff review ${name}`,
    '',
    '## Agent instructions',
    '',
    AGENT_INSTRUCTIONS,
...
const mergeTodos = (gitItems, todos) => {
