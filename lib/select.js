...
const parseParams = (body) => {
  if (!body) return [0];
  return body.split(';').map((part) => (part ? Number(part) : 0));
};
...
const applySgr = (params, state) => {
  ...
  while (i < params.length) {
    ...
    if (code === SGR_FG && hasRgb) {
      state.fg = [params[i + 2], params[i + 3], params[i + 4]];
      i += 5;
      continue;
    }
    if (code === SGR_BG && hasRgb) {
      state.bg = [params[i + 2], params[i + 3], params[i + 4]];
      i += 5;
      continue;
    }
    if (code === SGR_DEFAULT_FG) {
      state.fg = null;
      i += 1;
      continue;
    }
    if (code === SGR_DEFAULT_BG) {
      state.bg = null;
      i += 1;
      continue;
    }
