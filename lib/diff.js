...
const diffTokens = (a, b) => {
  ...
  const oldSpans = collapseSpans(oldParts);
  const newSpans = collapseSpans(newParts);
  if (isNoisy(oldSpans) || isNoisy(newSpans)) return wholeChanged(a, b);
  return { oldSpans, newSpans };
};
...
const alignChangeBlock = (dels, adds, delToAdd) => {
  ...
  const takeUnpairedAdds = (limit) => {
    while (addPos < limit) {
      const unpaired = addToDel[addPos] < 0;
      if (unpaired && !usedAdd[addPos]) {
        out.push(adds[addPos]);
        usedAdd[addPos] = true;
      }
      addPos += 1;
    }
...
const alignPairs = (lines) => {
  ...
  while (i < lines.length) {
    ...
    const aligned = alignChangeBlock(dels, adds, delToAdd);
    for (const line of aligned) out.push(line);
  }
  return out;
};
...
const alignSplitBlock = (dels, adds, delToAdd) => {
  ...
  const takeUnpairedAdds = (limit) => {
    while (addPos < limit) {
      ...
      if (free) {
        out.push({ left: null, right: adds[addPos] });
        usedAdd[addPos] = true;
      }
      addPos += 1;
    }
  ...
  for (let i = 0; i < dels.length; i++) {
    const j = delToAdd[i];
    if (j < 0) {
      out.push({ left: dels[i], right: null });
      continue;
    }
    takeUnpairedAdds(j);
    out.push({ left: dels[i], right: adds[j] });
    usedAdd[j] = true;
    if (addPos <= j) addPos = j + 1;
  }
  takeUnpairedAdds(adds.length);
  for (let j = 0; j < adds.length; j++) {
    if (!usedAdd[j]) out.push({ left: null, right: adds[j] });
  }
  return out;
};
...
const alignSplit = (lines) => {
  ...
  while (i < lines.length) {
    ...
    const aligned = alignSplitBlock(dels, adds, delToAdd);
    for (const row of aligned) out.push(row);
  }
  return out;
};
...
const parseDiff = (text) => {
  ...
  for (const raw of rawLines) {
    ...
    const mark = line[0];
    const isDiff = mark === ' ' || mark === '+' || mark === '-';
    const text = isDiff ? line.slice(1) : line;
    if (mark === '+') hunk.lines.push({ type: 'add', text, noNl: false });
    else if (mark === '-') hunk.lines.push({ type: 'del', text, noNl: false });
    else hunk.lines.push({ type: 'ctx', text, noNl: false });
  }
  closeFile(file, files);
  return files;
};
...
const displayLines = (hunk, blockId, layout = 'unified') => {
  const lines = [];
  for (const line of hunk.lines) {
    if (line.type === 'ctx' || line.blockId === blockId) {
      lines.push({ ...line });
    }
  }
  const painted = attachInline(lines);
  if (layout === 'mixed') return alignPairs(painted);
  if (layout === 'side') return alignSplit(painted);
  return painted;
};
  ...
  for (const file of files) {
    ...
    for (const rawHunk of file.hunks) {
      ...
      for (const block of blocks) {
        ...
        const patchAdd = formatPatch(file, hunk, blockId, 'old');
